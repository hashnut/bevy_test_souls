...
#include "GsMarketControllerWorld.h"
...
void FGsMarketControllerWorld::Initialize()
{
	_minItemGrade = FGsMarketHelper::GetWorldMarketMinimumItemGrade();

	Super::Initialize();
}

bool FGsMarketControllerWorld::IsTradableItem(const FGsSchemaItemCommon* InRow) const
{
	if (InRow &&
		InRow->tradable)
	{
		if (_minItemGrade <= InRow->grade)
		{
			return true;
		}
	}
...
void FGsMarketControllerWorld::SetItemGroupList(PD::GC::PKT_GC_ACK_WDMARKET_SALES_ITEM_LIST_READ* InPacket)
{
	...
	// 일부 리스트를 받는다
	for (PD::GC::PKT_GC_ACK_WDMARKET_SALES_ITEM_LIST_READ::SalesItemListIterator iter = InPacket->GetFirstSalesItemListIterator();
...
void FGsMarketControllerWorld::SetInstackableItemList(PD::GC::PKT_GC_ACK_WDMARKET_TRANSACTION_LIST_INSTACKABLE_READ* InPacket)
{
	// 전체 리스트를 받는다
	...
	for (PD::GC::PKT_GC_ACK_WDMARKET_TRANSACTION_LIST_INSTACKABLE_READ::TransactionListIterator iter = InPacket->GetFirstTransactionListIterator();
		iter != InPacket->GetLastTransactionListIterator(); ++iter)
	{
		...
		itemData->SetInstackableItemWd(itemId, &(*iter));
...
void FGsMarketControllerWorld::SetStackableItemList(PD::GC::PKT_GC_ACK_WDMARKET_TRANSACTION_LIST_STACKABLE_READ* InPacket)
{
	// 전체 리스트를 받는다
	ItemId itemId = InPacket->ItemId();
	FGsMarketItemGroupData* itemGroupData = GetItemGroupDataInter(itemId);
	...
	for (PD::GC::PKT_GC_ACK_WDMARKET_TRANSACTION_LIST_STACKABLE_READ::TransactionListIterator iter = InPacket->GetFirstTransactionListIterator();
		iter != InPacket->GetLastTransactionListIterator(); ++iter)
	{
		FGsMarketItemData* itemData = GetItemDataInter(iter->MarketTransactionId());
		if (nullptr == itemData)
		...
		itemData->SetStackableItemWd(itemId, &(*iter));
...
void FGsMarketControllerWorld::SetSettlementItemList(PD::GC::PKT_GC_ACK_WDMARKET_CALCULATE_LIST_READ* InPacket)
{
	_settlementList.Empty();

	for (PD::GC::PKT_GC_ACK_WDMARKET_CALCULATE_LIST_READ::CalculateListIterator iter = InPacket->GetFirstCalculateListIterator();
...
void FGsMarketControllerWorld::SetSellItemList(PD::GC::PKT_GC_ACK_WDMARKET_MY_TRANSACTION_LIST_READ* InPacket)
{
	_sellItemList.Empty();

	for (PD::GC::PKT_GC_ACK_WDMARKET_MY_TRANSACTION_LIST_READ::TransactionListIterator iter = InPacket->GetFirstTransactionListIterator();
		iter != InPacket->GetLastTransactionListIterator(); ++iter)
	{
		FGsMarketItemData* itemData = GetItemDataInter(iter->MarketTransactionId());
		if (nullptr == itemData)
...
void FGsMarketControllerWorld::SetItemPrice(PD::GC::PKT_GC_ACK_WDMARKET_ITEM_PRICE_READ* InPacket)
{
	_priceData.SetDataWd(InPacket);
}

void FGsMarketControllerWorld::SetSearchedList(PD::GC::PKT_GC_ACK_WDMARKET_ITEM_SEARCH_READ* InPacket)
{
	_serverSearchedItemGroupList.Empty();

	for (PD::GC::PKT_GC_ACK_WDMARKET_ITEM_SEARCH_READ::SalesItemListIterator iter = InPacket->GetFirstSalesItemListIterator();
		iter != InPacket->GetLastSalesItemListIterator(); ++iter)
	{
...
void FGsMarketControllerWorld::SetItemDetailInfo(PD::GC::PKT_GC_ACK_WDMARKET_TRANSACTION_DETAIL_EQUIP_READ* InPacket)
{
	FGsMarketItemData* itemData = GetItemDataInter(InPacket->MarketTransactionId());
	if (itemData)
	{
		itemData->SetDetailDataWd(InPacket);
	}

	FGsMarketSettlementData* settlementData = GetSettlementDataInter(InPacket->MarketTransactionId());
	if (settlementData)
	{
		settlementData->SetDetailDataWd(InPacket);
	}
}

void FGsMarketControllerWorld::SetBookmarkList(PD::GC::PKT_GC_ACK_WDMARKET_BOOKMARK_LIST_READ* InPacket)
{
	// pool에 반환
	...
	for (PD::GC::PKT_GC_ACK_WDMARKET_BOOKMARK_LIST_READ::MarketBookmarkListIterator iter = InPacket->GetFirstMarketBookmarkListIterator();
		iter != InPacket->GetLastMarketBookmarkListIterator(); ++iter)
	{
		if (FGsMarketItemGroupData* tempData = GetItemGroupDataInter(iter->ItemId()))
		{
			if (FGsMarketItemGroupData* itemGroupData = _poolItemGroupData.Claim())
			{
				...
				itemGroupData->SetDataBookmarkWd(&(*iter));
...
void FGsMarketControllerWorld::SetSettlementLogList(PD::GC::PKT_GC_ACK_WDMARKET_CALCULATED_LIST_READ* InPacket)
{
	...
	bool bIsEmpty = true;
	for (PD::GC::PKT_GC_ACK_WDMARKET_CALCULATED_LIST_READ::CalculatedListIterator iter = InPacket->GetFirstCalculatedListIterator();
...
void FGsMarketControllerWorld::SetSellRegisteredItem(PD::SC::PKT_SC_ACK_WDMARKET_TRANSACTION_REGISTER_READ* InPacket)
{
	FGsMarketItemData* itemData = GetItemDataInter(InPacket->MarketTransactionId());
	if (nullptr == itemData)
	{
	...
	itemData->SetSellRegisteredItemWd(InPacket);
