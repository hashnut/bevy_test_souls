...
class T1PROJECT_API FGsStateLobbyRepairCheck : public FGsContentsLobbyBase
{
...
public:
	FGsStateLobbyRepairCheck();
	virtual ~FGsStateLobbyRepairCheck();
...
protected:
	void OnRepairCheckFailed();

private:
	// 다운로드 실패
	void RepairCheckResultFailed(const IGsMessageParam* InParam);
	// 다운로드 성공
	void RepairCheckResultSucceeded(const IGsMessageParam* InParam);
};
