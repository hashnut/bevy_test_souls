...
FGsStateLobbyPatch::FGsStateLobbyPatch() : FGsContentsLobbyBase(FGsContentsMode::InLobby::PATCH)
{
}

FGsStateLobbyPatch::~FGsStateLobbyPatch()
{
}

void FGsStateLobbyPatch::Enter()
{
	FGsContentsLobbyBase::Enter();

	GSLOG(Log, TEXT("FGsStateLobbyPatch::Enter()"));
	...
	FText::FindText(TEXT("TitleSceneText"), TEXT("PatchStateBuildVerification"), _textPatchStateBuildVerification);
...
void FGsStateLobbyPatch::RequestPatch()
{
	GSLOG(Log, TEXT("FGsStateLobbyPatch::RequestPatch() Call"));

	GPatch()->PatchProcessRequest();
}

void FGsStateLobbyPatch::OnPatchSucceeded()
{
	// 다운로드 성공
	GSLOG(Log, TEXT("FGsStateLobbyPatch::OnPatchSucceeded"));
...
void FGsStateLobbyPatch::OnPatchFailed()
{
	// 다운로드 실패
	GSLOG(Log, TEXT("FGsStateLobbyPatch::OnPatchFailed"));

	// 패치 인스톨 실패가 아닐때는 기존 로직
	if (GPatch()->IsInstallFail() == false)
	{
		FGsLobbyFlowMsgParam param(MessageStage::FAILED_PATCH);
		GMessage()->GetStageParam().SendMessage(MessageStage::LOBBY_STATE_UPDATE_NOTIFY, &param);
	}
	else
	{
		switch (GPatch()->GetInstallFailError())
		{
			// 등록된 오류가 없습니다.
			...
			// 아래 내용은 협의, 혹은 기능상 처리가 필요 없어서 주석처리했지만, 혹시나 관련 문제가 나왔을때 처리를 하기 위해 일단 주석처리
			...
			//// 전제 조건 설치 프로그램이 실패했습니다.
			//// EpicGamesLauncher 사용하여 다운로드시 UE4PrereqSetup_x64.exe 레지스터리 오류로 삭제해야지 되는 오류(그후 다시 제설치됨)
			//// 우리는 상관없는 부분이지만 만약을 위해
			...
			//// 초기화 오류입니다.
			//// EpicGamesLauncher 오류 우리는 사용하지 않아서 올일이 없음
			//// 우리는 상관없는 부분이지만 만약을 위해
			...
			// 과도한 경로 길이로 인해 파일 생성 오류가 발생했습니다.
			// 윈도우 10에 경우 최대 260자인데 우리는 경로를 지정하는 시스템인데 런처에서 설치시 처리되는게 있는지 확인
			/*case EBuildPatchInstallError::PathLengthExceeded:
			{
			} break;*/
			//case EBuildPatchInstallError::NumInstallErrors:
			//{
			//	// 다시 패치 시도 할수 있는 상태로
			//	FGsLobbyFlowMsgParam param(MessageStage::FAILED_PATCH);
			//	GMessage()->GetStageParam().SendMessage(MessageStage::LOBBY_STATE_UPDATE_NOTIFY, &param);
...
void FGsStateLobbyPatch::Update(float In_deltaTime)
{
	static uint8 logCounter = 0;
...
void FGsStateLobbyPatch::Init()
{
	FGsContentsLobbyBase::Init();	
}

void FGsStateLobbyPatch::Close()
{
	FGsContentsLobbyBase::Close();
}

void FGsStateLobbyPatch::RegisterMessages()
{
	...
	_msgHandleLobbySystemList.Emplace(mLobbySystem.AddRaw(MessageSystem::PATCH_FALSE,
		this, &FGsStateLobbyPatch::PatchResultFailed));
	_msgHandleLobbySystemList.Emplace(mLobbySystem.AddRaw(MessageSystem::PATCH_TRUE,
		this, &FGsStateLobbyPatch::PatchResultSucceeded));
	_msgHandleLobbySystemList.Emplace(mLobbySystem.AddRaw(MessageSystem::PATCH_Click,
		this, &FGsStateLobbyPatch::PatchClickEvent));
}

void FGsStateLobbyPatch::UnregisterMessages()
{
...
// 다운로드 실패
void FGsStateLobbyPatch::PatchResultFailed(const IGsMessageParam* InParam)
{
	GSLOG(Log, TEXT("FGsStateLobbyPatch::PatchResultFailed"));
	OnPatchFailed();
}

// 다운로드 성공
void FGsStateLobbyPatch::PatchResultSucceeded(const IGsMessageParam* InParam)
{
	OnPatchSucceeded();
}

void FGsStateLobbyPatch::StartTimer()
{
	...
	if (nullptr != world)
	{
		world->GetTimerManager().SetTimer(_timerHandle,
			FTimerDelegate::CreateRaw(this, &FGsStateLobbyPatch::EndTimer),
			5.0f, false);
	}
}

void FGsStateLobbyPatch::EndTimer()
{
...
void FGsStateLobbyPatch::StopTimer()
{
...
void FGsStateLobbyPatch::PatchClickEvent(const IGsMessageParam* InParam)
{
	GSLOG(Log, TEXT("FGsStateLobbyPatch::PatchClickEvent"));
	OnPatchFailed();
}