...
#include "GsTutorialActionHUDQuestTeleport.h"
...
bool FGsTutorialActionHUDQuestTeleport::OnStart()
{
	...
	_questUI->OnTutorialQuestListEvent.BindRaw(this, &FGsTutorialActionHUDQuestTeleport::OnTutorialQuestListEvent);
	_questUI->SetTutorialTarget(idTable->targetId, false);

	return true;
}

void FGsTutorialActionHUDQuestTeleport::OnEnd(bool bInSendMessage)
{
	if (_questUI.IsValid())
	{
...
void FGsTutorialActionHUDQuestTeleport::OnTutorialQuestListEvent(UWidget* InTargetWidget)
{
	if (false == _questUI.IsValid())
	{
		OnEnd();
