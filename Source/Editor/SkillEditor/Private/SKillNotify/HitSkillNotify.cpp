...
#if WITH_EDITOR
void UHitSkillNotify::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UHitSkillNotify::Notify(class USkeletalMeshComponent* MeshComp, class UAnimSequenceBase* Animation)
{		
	...
	if (UseSkillNotifyCollision)
	{
		if (SkillNotifySet.GetRow())
		{
			...
			for (const FGsSchemaSkillCollisionRow& el : skillNotifySet->collisionInfoList)
			{
				...
				DrawData.Color = FLinearColor(NotifyColor);// FColor(192, 128, 128, 128); //NotifyColor;
