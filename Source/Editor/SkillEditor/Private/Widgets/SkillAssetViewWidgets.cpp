#include "SkillAssetViewWidgets.h"
...
#include "SkillEditorPathViewTypes.h"
...
FReply FSkillAssetViewModeUtils::OnViewModeKeyDown( const TSet< TSharedPtr<FSkillAssetViewItem> >& SelectedItems, const FKeyEvent& InKeyEvent )
{
	...
	if ( InKeyEvent.IsControlDown() && InKeyEvent.GetCharacter() == 'C' )
	{
		...
		{
			const TSharedPtr<FSkillAssetViewItem>& Item = *ItemIt;
			if(Item.IsValid())
			{
				if(Item->GetType() == ESkillAssetItemType::Folder)
				{
					// we need to recurse & copy references to all folder contents
					FARFilter Filter;
					Filter.PackagePaths.Add(*StaticCastSharedPtr<FSkillAssetViewFolder>(Item)->FolderPath);
				...
				{
					SelectedAssets.Add(StaticCastSharedPtr<FSkillAssetViewAsset>(Item)->Data);
				}
			}
		}
...
TSharedRef<SWidget> FSkillAssetViewItemHelper::CreateListItemContents(SSkillAssetListItem* const InListItem, const TSharedRef<SWidget>& InThumbnail, FName& OutItemShadowBorder)
{
	return CreateListTileItemContents(InListItem, InThumbnail, OutItemShadowBorder);
}

//TSharedRef<SWidget> FSkillAssetViewItemHelper::CreateTileItemContents(SSkillAssetTileItem* const InTileItem, const TSharedRef<SWidget>& InThumbnail, FName& OutItemShadowBorder)
//{
//	return CreateListTileItemContents(InTileItem, InThumbnail, OutItemShadowBorder);
//}

template <typename T>
TSharedRef<SWidget> FSkillAssetViewItemHelper::CreateListTileItemContents(T* const InTileOrListItem, const TSharedRef<SWidget>& InThumbnail, FName& OutItemShadowBorder)
{
	...
	if (InTileOrListItem->IsFolder())
	{
		...
		TSharedPtr<FSkillAssetViewFolder> AssetFolderItem = StaticCastSharedPtr<FSkillAssetViewFolder>(InTileOrListItem->AssetItem);
...
public:
	...
	{ }
		...
		SLATE_ARGUMENT(TSharedPtr<SSkillAssetViewItem>, AssetViewItem)
	...
	{
		TSharedPtr<SSkillAssetViewItem> AssetViewItemPin = AssetViewItem.Pin();
...
private:
	TWeakPtr<SSkillAssetViewItem> AssetViewItem;
};
...
FReply SSkillAssetListView::OnKeyDown(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
	FReply Reply = FSkillAssetViewModeUtils::OnViewModeKeyDown(SelectedItems, InKeyEvent);
	...
	{
		return SListView<TSharedPtr<FSkillAssetViewItem>>::OnKeyDown(InGeometry, InKeyEvent);
	}
}

void SSkillAssetListView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	// Regreshing an asset view is an intensive task. Do not do this while a user
	// is dragging arround content for maximum responsiveness.
	// Also prevents a re-entrancy crash caused by potentially complex thumbnail generators.
	if (!FSlateApplication::Get().IsDragDropping())
	{
		SListView<TSharedPtr<FSkillAssetViewItem>>::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);
	}
}
...
SSkillAssetViewItem::~SSkillAssetViewItem()
{
...
void SSkillAssetViewItem::Construct( const FArguments& InArgs )
{
	...
	AssetItem->OnAssetDataChanged.AddSP(this, &SSkillAssetViewItem::OnAssetDataChanged);
	...
	ISourceControlModule::Get().RegisterProviderChanged(FSourceControlProviderChanged::FDelegate::CreateSP(this, &SSkillAssetViewItem::HandleSourceControlProviderChanged));
	SourceControlStateChangedDelegateHandle = ISourceControlModule::Get().GetProvider().RegisterSourceControlStateChanged_Handle(FSourceControlStateChanged::FDelegate::CreateSP(this, &SSkillAssetViewItem::HandleSourceControlStateChanged));
...
void SSkillAssetViewItem::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
TSharedPtr<IToolTip> SSkillAssetViewItem::GetToolTip()
{
	return ShouldAllowToolTip.Get() ? SCompoundWidget::GetToolTip() : NULL;
}

bool SSkillAssetViewItem::ValidateDragDrop( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent, bool& OutIsKnownDragOperation ) const
{
	OutIsKnownDragOperation = false;
	return OutIsKnownDragOperation;
	/*OutIsKnownDragOperation = false;
	return IsFolder() && DragDropHandler::ValidateDragDropOnAssetFolder(MyGeometry, DragDropEvent, StaticCastSharedPtr<FSkillAssetViewFolder>(AssetItem)->FolderPath, OutIsKnownDragOperation);*/
}

void SSkillAssetViewItem::OnDragEnter( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	ValidateDragDrop(MyGeometry, DragDropEvent, bDraggedOver); // updates bDraggedOver
}
	
void SSkillAssetViewItem::OnDragLeave( const FDragDropEvent& DragDropEvent )
{
	if(IsFolder())
	{
...
FReply SSkillAssetViewItem::OnDragOver( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	ValidateDragDrop(MyGeometry, DragDropEvent, bDraggedOver); // updates bDraggedOver
	return (bDraggedOver) ? FReply::Handled() : FReply::Unhandled();
}

FReply SSkillAssetViewItem::OnDrop( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	if (ValidateDragDrop(MyGeometry, DragDropEvent, bDraggedOver)) // updates bDraggedOver
	{
		bDraggedOver = false;

		check(AssetItem->GetType() == ESkillAssetItemType::Folder);
		...
		if (Operation->IsOfType<FExternalDragOperation>())
		{
			...
			OnFilesDragDropped.ExecuteIfBound(DragDropOp->GetFiles(), StaticCastSharedPtr<FSkillAssetViewFolder>(AssetItem)->FolderPath);
			return FReply::Handled();
		}
		...
		{
			TSharedPtr<FAssetDragDropOp> DragDropOp = StaticCastSharedPtr<FAssetDragDropOp>(Operation);
			OnAssetsOrPathsDragDropped.ExecuteIfBound(DragDropOp->GetAssets(), DragDropOp->GetAssetPaths(), StaticCastSharedPtr<FSkillAssetViewFolder>(AssetItem)->FolderPath);
			return FReply::Handled();
		}
	}
...
bool SSkillAssetViewItem::IsNameReadOnly() const
{
	...
	//	return !ContentBrowserUtils::CanRenameAsset(StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data);
	//}
	//else
	//{
	//	// Read-only if we can't be renamed
	//	return !ContentBrowserUtils::CanRenameFolder(StaticCastSharedPtr<FSkillAssetViewFolder>(AssetItem)->FolderPath);
	//}

	return false;
}

void SSkillAssetViewItem::HandleBeginNameChange( const FText& OriginalText )
{
	OnRenameBegin.ExecuteIfBound(AssetItem, OriginalText.ToString(), LastGeometry.GetLayoutBoundingRect());
}

void SSkillAssetViewItem::HandleNameCommitted( const FText& NewText, ETextCommit::Type CommitInfo )
{
	OnRenameCommit.ExecuteIfBound(AssetItem, NewText.ToString(), LastGeometry.GetLayoutBoundingRect(), CommitInfo);
}

bool SSkillAssetViewItem::HandleVerifyNameChanged( const FText& NewText, FText& OutErrorMessage )
{
...
void SSkillAssetViewItem::OnAssetDataChanged()
{
	CachePackageName();
	...
	if ( AssetItem->GetType() != ESkillAssetItemType::Folder )
	{
		UClass* AssetClass = FindObject<UClass>(ANY_PACKAGE, *StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data.AssetClass.ToString());
		if (AssetClass)
		{
...
void SSkillAssetViewItem::DirtyStateChanged()
{
}

FText SSkillAssetViewItem::GetAssetClassText() const
{
	if (!AssetItem.IsValid())
	{
		return FText();
	...
	return FText::FromName(StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data.AssetClass);
}

const FSlateBrush* SSkillAssetViewItem::GetSCCStateImage() const
{
	return ThumbnailEditMode.Get() ? FEditorStyle::GetNoBrush() : SCCStateBrush;
}

void SSkillAssetViewItem::HandleSourceControlProviderChanged(ISourceControlProvider& OldProvider, ISourceControlProvider& NewProvider)
{
	OldProvider.UnregisterSourceControlStateChanged_Handle(SourceControlStateChangedDelegateHandle);
	SourceControlStateChangedDelegateHandle = NewProvider.RegisterSourceControlStateChanged_Handle(FSourceControlStateChanged::FDelegate::CreateSP(this, &SSkillAssetViewItem::HandleSourceControlStateChanged));
...
void SSkillAssetViewItem::HandleSourceControlStateChanged()
{
	if ( ISourceControlModule::Get().IsEnabled() && AssetItem.IsValid() && (AssetItem->GetType() == ESkillAssetItemType::Normal) && !AssetItem->IsTemporaryItem() && !FPackageName::IsScriptPackage(CachedPackageName) )
	{
...
const FSlateBrush* SSkillAssetViewItem::GetDirtyImage() const
{
	return IsDirty() ? AssetDirtyBrush : NULL;
}

EVisibility SSkillAssetViewItem::GetThumbnailEditModeUIVisibility() const
{
	return !IsFolder() && ThumbnailEditMode.Get() ? EVisibility::Visible : EVisibility::Collapsed;
}

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
TSharedRef<SWidget> SSkillAssetViewItem::CreateToolTipWidget() const
{
	if ( AssetItem.IsValid() )
	{
		bool bTryCustomAssetToolTip = true;
		if (OnIsAssetValidForCustomToolTip.IsBound() && AssetItem->GetType() != ESkillAssetItemType::Folder)
		{
			FAssetData& AssetData = StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data;
			bTryCustomAssetToolTip = OnIsAssetValidForCustomToolTip.Execute(AssetData);
		}

		if(bTryCustomAssetToolTip && OnGetCustomAssetToolTip.IsBound() && AssetItem->GetType() != ESkillAssetItemType::Folder)
		{
			FAssetData& AssetData = StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data;
			return OnGetCustomAssetToolTip.Execute(AssetData);
		}
		else if(AssetItem->GetType() != ESkillAssetItemType::Folder)
		{
			const FAssetData& AssetData = StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data;
			...
			OverallTooltipVBox->AddSlot()
				...
				[
					...
					[
						...
						[
							SNew(STextBlock)
							.Visibility(this, &SSkillAssetViewItem::GetCheckedOutByOtherTextVisibility)
							.Text(this, &SSkillAssetViewItem::GetCheckedOutByOtherText)
		...
		{
			const FText& FolderName = StaticCastSharedPtr<FSkillAssetViewFolder>(AssetItem)->FolderName;
			const FString& FolderPath = StaticCastSharedPtr<FSkillAssetViewFolder>(AssetItem)->FolderPath;
...
EVisibility SSkillAssetViewItem::GetCheckedOutByOtherTextVisibility() const
{
	return GetCheckedOutByOtherText().IsEmpty() ? EVisibility::Collapsed : EVisibility::Visible;
}

FText SSkillAssetViewItem::GetCheckedOutByOtherText() const
{
	if ( AssetItem.IsValid() && AssetItem->GetType() != ESkillAssetItemType::Folder && !GIsSavingPackage && !IsGarbageCollecting() )
	{
		const FAssetData& AssetData = StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data;
...
FText SSkillAssetViewItem::GetAssetUserDescription() const
{
...
void SSkillAssetViewItem::AddToToolTipInfoBox(const TSharedRef<SVerticalBox>& InfoBox, const FText& Key, const FText& Value, bool bImportant) const
{
...
void SSkillAssetViewItem::UpdatePackageDirtyState()
{
	bool bNewIsDirty = false;

	// Only update the dirty state for non-temporary asset items that aren't a built in script
	if ( AssetItem.IsValid() && !AssetItem->IsTemporaryItem() && AssetItem->GetType() != ESkillAssetItemType::Folder && !FPackageName::IsScriptPackage(CachedPackageName) )
	{
...
bool SSkillAssetViewItem::IsDirty() const
{
	return bPackageDirty;
}

void SSkillAssetViewItem::UpdateSourceControlState(float InDeltaTime)
{
	...
	if ( !bSourceControlStateRequested && SourceControlStateDelay > 1.0f && ISourceControlModule::Get().IsEnabled() && AssetItem.IsValid() )
	{
		...
		if ( !AssetItem->IsTemporaryItem() && AssetItem->GetType() != ESkillAssetItemType::Folder && !FPackageName::IsScriptPackage(CachedPackageName) && !FPackageName::IsMemoryPackage(CachedPackageName) && !FPackageName::IsTempPackage(CachedPackageName))
		{
...
void SSkillAssetViewItem::CachePackageName()
{
	if(AssetItem.IsValid())
	{
		if(AssetItem->GetType() != ESkillAssetItemType::Folder)
		{
			CachedPackageName = StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data.PackageName.ToString();
			CachedPackageFileName = SourceControlHelpers::PackageFilename(CachedPackageName);
		}
		else
		{
			CachedPackageName = StaticCastSharedPtr<FSkillAssetViewFolder>(AssetItem)->FolderName.ToString();
		}
	}
}
...
#if BSAM_TEMP
#else
void SSkillAssetViewItem::CacheDisplayTags()
{
	...
	for (const auto& TagAndValuePair : AssetData.TagsAndValues)
	{
		...
		FText DisplayValue;
		{
			...
			if (!bHasSetDisplayValue && FTextStringHelper::IsComplexText(*TagAndValuePair.Value))
			{
				...
				FTextStringHelper::ReadFromBuffer(*TagAndValuePair.Value, DisplayValue);
			}
			...
			if (!bHasSetDisplayValue)
			{
				...
				if (TagField)
				{
					...
					if (FByteProperty* ByteProp = Cast<FByteProperty>(TagField))
					{
						TagProp = ByteProp;
						TagEnum = ByteProp->Enum;
					}
					else if (FEnumProperty* EnumProp = Cast<FEnumProperty>(TagField))
					{
						TagProp = EnumProp;
...
const FSlateBrush* SSkillAssetViewItem::GetBorderImage() const
{
	return bDraggedOver ? FEditorStyle::GetBrush("Menu.Background") : FEditorStyle::GetBrush("NoBorder");
}

bool SSkillAssetViewItem::IsFolder() const
{
	return AssetItem.IsValid() && AssetItem->GetType() == ESkillAssetItemType::Folder;
}

FText SSkillAssetViewItem::GetNameText() const
{
	if(AssetItem.IsValid())
	{
		if(AssetItem->GetType() != ESkillAssetItemType::Folder)
		{
			return FText::FromName(StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data.AssetName);
		}
		else
		{
			return StaticCastSharedPtr<FSkillAssetViewFolder>(AssetItem)->FolderName;
		}
	}
...
FSlateColor SSkillAssetViewItem::GetAssetColor() const
{
	if(AssetItem.IsValid())
	{
		if(AssetItem->GetType() == ESkillAssetItemType::Folder)
		{
			TSharedPtr<FSkillAssetViewFolder> AssetFolderItem = StaticCastSharedPtr<FSkillAssetViewFolder>(AssetItem);
...
void SSkillAssetViewItem::SetForceMipLevelsToBeResident(bool bForce) const
{
	if(AssetItem.IsValid() && AssetItem->GetType() == ESkillAssetItemType::Normal)
	{
		const FAssetData& AssetData = StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data;
...
void SSkillAssetViewItem::HandleAssetLoaded(UObject* InAsset) const
{
	if(InAsset != nullptr)
	{
		if(AssetItem.IsValid() && AssetItem->GetType() == ESkillAssetItemType::Normal)
...
bool SSkillAssetViewItem::OnVisualizeTooltip(const TSharedPtr<SWidget>& TooltipContent)
{
	if(OnVisualizeAssetToolTip.IsBound() && TooltipContent.IsValid() && AssetItem->GetType() != ESkillAssetItemType::Folder)
	{
		FAssetData& AssetData = StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data;
...
void SSkillAssetViewItem::OnToolTipClosing()
{
...
SSkillAssetListItem::~SSkillAssetListItem()
{
	FCoreUObjectDelegates::OnAssetLoaded.RemoveAll(this);
}

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void SSkillAssetListItem::Construct( const FArguments& InArgs )
{
	SSkillAssetViewItem::Construct( SSkillAssetViewItem::FArguments()
		.AssetItem(InArgs._AssetItem)
		.OnRenameBegin(InArgs._OnRenameBegin)
		.OnRenameCommit(InArgs._OnRenameCommit)
		.OnVerifyRenameCommit(InArgs._OnVerifyRenameCommit)
	...
	if ( AssetItem.IsValid() && AssetThumbnail.IsValid() )
	{
		...
		ThumbnailConfig.bForceGenericThumbnail = (AssetItem->GetType() == ESkillAssetItemType::Creation);
		ThumbnailConfig.bAllowAssetSpecificThumbnailOverlay = (AssetItem->GetType() != ESkillAssetItemType::Creation);
	...
	TSharedRef<SWidget> ItemContents = FSkillAssetViewItemHelper::CreateListItemContents(this, Thumbnail.ToSharedRef(), ItemShadowBorderName);

	ChildSlot
	[
		SNew(SBorder)
		.BorderImage(this, &SSkillAssetViewItem::GetBorderImage)
		.Padding(0)
		.AddMetaData<FTagMetaData>(FTagMetaData(AssetItem->GetType() == ESkillAssetItemType::Normal ? StaticCastSharedPtr<FSkillAssetViewAsset>(AssetItem)->Data.ObjectPath : NAME_None))
		[
			SNew(SHorizontalBox)
			...
			[
				...
				.WidthOverride( this, &SSkillAssetListItem::GetThumbnailBoxSize )
				.HeightOverride( this, &SSkillAssetListItem::GetThumbnailBoxSize )
				[
					// Drop shadow border
					SNew(SBorder)
					.Padding(4.f)
			...
			[
				...
				[
					...
					.Text( GetNameText() )
					.OnBeginTextEdit(this, &SSkillAssetListItem::HandleBeginNameChange)
					.OnTextCommitted(this, &SSkillAssetListItem::HandleNameCommitted)
					.OnVerifyTextChanged(this, &SSkillAssetListItem::HandleVerifyNameChanged)
					.HighlightText(InArgs._HighlightText)
					.IsSelected(InArgs._IsSelected)
					.IsReadOnly(this, &SSkillAssetListItem::IsNameReadOnly)
				]
	...
	FCoreUObjectDelegates::OnAssetLoaded.AddSP(this, &SSkillAssetViewItem::HandleAssetLoaded);
}
END_SLATE_FUNCTION_BUILD_OPTIMIZATION

void SSkillAssetListItem::OnAssetDataChanged()
{
	SSkillAssetViewItem::OnAssetDataChanged();
...
FOptionalSize SSkillAssetListItem::GetThumbnailBoxSize() const
{
	return FOptionalSize( ItemHeight.Get() );
}

FOptionalSize SSkillAssetListItem::GetSCCImageSize() const
{
