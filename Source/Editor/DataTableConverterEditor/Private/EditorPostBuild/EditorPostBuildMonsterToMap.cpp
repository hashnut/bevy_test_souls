...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(EditorPostBuildMonsterToMap, TEXT("Raon.EditorPostBuild.4 MonsterToMap")
	, EAutomationTestFlags::EditorContext | EAutomationTestFlags::MediumPriorityAndAbove | EAutomationTestFlags::EngineFilter)

bool EditorPostBuildMonsterToMap::RunTest(const FString& Parameters)
{
	const FString worldMapPath = TEXT("/Game/DataCenter/DataTable/MonsterDropItem/DTGsMonsterToMapData");

	// 원본 데이터테이블 로드
	UGsDataTable* OriginalDataTable = nullptr;
	if (!FGsDataTableConvertBase::LoadDataTable(worldMapPath, OriginalDataTable))
	{
		return false;
	}
	...
	if (FTerritoryEditor* instance = FModuleManager::GetModulePtr<FTerritoryEditor>("TerritoryEditor"))
	{
		...
		if (manager)
		{
			TWeakPtr<FGsTerritoryDataBase> monsterToMap = manager->GetMonsterToMapData();
			if (monsterToMap.IsValid())
			{
				monsterToMap.Pin()->Save(0, nullptr, true, nullptr, "");
			}
		}
	}
