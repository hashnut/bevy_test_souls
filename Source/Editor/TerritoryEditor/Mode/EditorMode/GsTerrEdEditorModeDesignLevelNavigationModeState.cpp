#include "GsTerrEdEditorModeDesignLevelNavigationModeState.h"
...
#include "DesignLevelNavigationMode/GsTerrEdDesignLevelNavigationMapIdleModeState.h"
#include "DesignLevelNavigationMode/GsTerrEdDesignLevelNavigationMapChangeModeState.h"
#include "DesignLevelNavigationMode/GsTerrEdDesignLevelNavigationMapOpenModeState.h"
#include "DesignLevelNavigationMode/GsTerrEdDesignLevelNavigationNewCurrentLevelModeState.h"
#include "DesignLevelNavigationMode/GsTerrEdDesignLevelNavigationPostSaveModeState.h"

#define LOCTEXT_NAMESPACE	"TerritoryEditor"

FGsTerrEdEditorModeDesignLevelNavigationModeState::FGsTerrEdEditorModeDesignLevelNavigationModeState()
{
	//맵 대기
	FGsTerrEdDesignLevelNavigationMapIdleModeState* mapIdleMode = new FGsTerrEdDesignLevelNavigationMapIdleModeState();
	mapIdleMode->SetParents(this);
	AddState(TERR_ED_EDITOR_MODE_MAP_IDLE_MODE, MakeShareable(mapIdleMode));

	//맵 로드
	FGsTerrEdDesignLevelNavigationMapChangeModeState* mapChangeMode = new FGsTerrEdDesignLevelNavigationMapChangeModeState();
	mapChangeMode->SetParents(this);
	AddState(TERR_ED_EDITOR_MODE_MAP_CHANGE_MODE, MakeShareable(mapChangeMode));

	//맵 오픈 
	FGsTerrEdDesignLevelNavigationMapOpenModeState* mapOpenMode = new FGsTerrEdDesignLevelNavigationMapOpenModeState();
	mapOpenMode->SetParents(this);
	AddState(TERR_ED_EDITOR_MODE_MAP_OPEN_MODE, MakeShareable(mapOpenMode));

	//서버 레벨 설정
	FGsTerrEdDesignLevelNavigationNewCurrentLevelModeState* newCurrentLevel = new FGsTerrEdDesignLevelNavigationNewCurrentLevelModeState();
	newCurrentLevel->SetParents(this);
	AddState(TERR_ED_EDITOR_MODE_NEW_CURRENT_LEVEL_MODE, MakeShareable(newCurrentLevel));

	//저장 후 호출
	FGsTerrEdDesignLevelNavigationPostSaveModeState* postSaveLevel = new FGsTerrEdDesignLevelNavigationPostSaveModeState();
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::Enter()
{
	_currentLevelId = 0;
	_currentLevelName = "";
	...
	_mapChangedHandle = FEditorDelegates::MapChange.AddRaw(this, &FGsTerrEdEditorModeDesignLevelNavigationModeState::OnMapChange);
	...
	_mapOpenedHandle = FEditorDelegates::OnMapOpened.AddRaw(this, &FGsTerrEdEditorModeDesignLevelNavigationModeState::OnMapOpend);
	...
	_newLevelLoadedHandle = FEditorDelegates::NewCurrentLevel.AddRaw(this, &FGsTerrEdEditorModeDesignLevelNavigationModeState::OnNewCurrentLevel);
	...
	_postSaveHandle = FEditorDelegates::PostSaveWorld.AddRaw(this, &FGsTerrEdEditorModeDesignLevelNavigationModeState::OnPostSaveLevel);
	...
	_additionalEvent.AddRaw(this, &FGsTerrEdEditorModeDesignLevelNavigationModeState::OnTest);
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::Exit()
{
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::Tick(float inTick)
{
	if (false == _isTick)
	{
		return;
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::MapChange()
{
	ChangeState(TERR_ED_EDITOR_MODE_MAP_CHANGE_MODE);
}

void FGsTerrEdEditorModeDesignLevelNavigationModeState::MapOpened(const FString& inFilename)
{
	ChangeState(TERR_ED_EDITOR_MODE_MAP_OPEN_MODE);
}

//Open server level
void FGsTerrEdEditorModeDesignLevelNavigationModeState::NewCurrentLevel()
{
	ChangeState(TERR_ED_EDITOR_MODE_NEW_CURRENT_LEVEL_MODE);
}

void FGsTerrEdEditorModeDesignLevelNavigationModeState::PostSave()
{
	ChangeState(TERR_ED_EDITOR_MODE_POST_SAVE_MODE);
}

void FGsTerrEdEditorModeDesignLevelNavigationModeState::FinishNavigationDesignLevel()
{
	FText okTitleText = FText::FromString("Finish save all design level");
	FText okMessageText = FText::FromString("Finish save all design level");
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::FinishNavigationCurrentDesignLevel()
{
...
/// <returns></returns>
bool FGsTerrEdEditorModeDesignLevelNavigationModeState::OpenLevel(const FString& inLevelName)
{
	UE_LOG(LogTerritoryEditor, Log, TEXT("FGsTerrEdEditorModeDesignLevelNavigationModeState::OpenLevel - Start"));
...
맵이 오픈 되고 난 후 콜백
*/
void FGsTerrEdEditorModeDesignLevelNavigationModeState::LoadDesignStreamingLevel()
{	
	UE_LOG(LogTerritoryEditor, Log, TEXT("FGsTerrEdEditorModeDesignLevelNavigationModeState::OnOpenLevelForSaveAllMap - OnOpenLevel - level index : %d\tlevel key : %d"), _currentLevelIndex, _currentLevelId);
	...
	if (false == UGsTerritoryLevelUtil::TryGetWorkLevels(levelCollection, levelList, world, EGsTerritoryLevelType::Design))
	{
		FinishNavigationCurrentDesignLevel();
		return;
	}
	...
	for (TSharedPtr<FLevelModel> level : levelList)
	{
		...
		FString levelKey;
		if (false == UGsTerritoryLevelUtil::IsValidDesignLevelName(levelName, persistantLevel, levelKey))
		{
			continue;
		}
	...
	UE_LOG(LogTerritoryEditor, Log, TEXT("FGsTerrEdEditorModeDesignLevelNavigationModeState::OnOpenLevelForSaveAllMap - OnOpenLevel End"));
}


//서버 레벨 추가시 콜백
void FGsTerrEdEditorModeDesignLevelNavigationModeState::SaveDesignStreamingLevel()
{
	UE_LOG(LogTerritoryEditor, Log, TEXT("FGsTerrEdEditorModeDesignLevelNavigationModeState::SaveServerStreamingLevel - OnLoadedServerLevel - Start"));
	...
	FString levelName;
	FString levelKey;
	if (false == UGsTerritoryLevelUtil::IsValidDesignLevel(level, levelName, levelKey)
	...
	UE_LOG(LogTerritoryEditor, Log, TEXT("FGsTerrEdEditorModeDesignLevelNavigationModeState::SaveServerStreamingLevel - OnAddServerLevel - start"));
	...
	SaveDesignLevelInternal();

	UE_LOG(LogTerritoryEditor, Log, TEXT("FGsTerrEdEditorModeDesignLevelNavigationModeState::SaveServerStreamingLevel - OnLoadedServerLevel - End"));
}

void FGsTerrEdEditorModeDesignLevelNavigationModeState::SaveDesignLevelInternal()
{
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::OnMapChange(uint32 inFalg)
{
	MapChange();
}

void FGsTerrEdEditorModeDesignLevelNavigationModeState::OnMapOpend(const FString& inFilename, bool inIsTemplate)
{
	MapOpened(inFilename);
}

void FGsTerrEdEditorModeDesignLevelNavigationModeState::OnNewCurrentLevel()
{
	NewCurrentLevel();
}


void FGsTerrEdEditorModeDesignLevelNavigationModeState::OnPostSaveLevel(uint32 inSaveFlag, UWorld* inWorld, bool inSuccess)
{
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::ExcuteEvent()
{
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::OnTest()
{
	UE_LOG(LogTemp, Log, TEXT("FGsTerrEdEditorModeDesignLevelNavigationModeState::OnTest"));
}

void FGsTerrEdEditorModeDesignLevelNavigationModeState::SaveLevel()
{
	UWorld* world = GEditor->GetEditorWorldContext().World();
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::SetCurrentLevelName(const FString& inServerLevel)
{
	_currentLevelName = inServerLevel;
}
...
void FGsTerrEdEditorModeDesignLevelNavigationModeState::OnLevelChange()
{
	UWorld* world = GEditor->GetEditorWorldContext().World();
	if (_levelChangeHandle.IsValid())
	{
