...
TSharedPtr<FSlateStyleSet> FGsTerritoryEditorStyle::_styleInstance = NULL;

void FGsTerritoryEditorStyle::Initialize()
{
	if (!_styleInstance.IsValid())
	{
		_styleInstance = Create();
		FSlateStyleRegistry::RegisterSlateStyle(*_styleInstance.Get());
	}
}

void FGsTerritoryEditorStyle::Shutdown()
{
	FSlateStyleRegistry::UnRegisterSlateStyle(*_styleInstance);
	ensure(_styleInstance.IsUnique());
	_styleInstance.Reset();
}

void FGsTerritoryEditorStyle::ReloadTextures()
{
...
FName FGsTerritoryEditorStyle::GetStyleSetName()
{
	static FName StyleSetName(TEXT("TerritoryEditorStyle"));
	return StyleSetName;
}
...
TSharedRef<class FSlateStyleSet> FGsTerritoryEditorStyle::Create()
{
	TSharedRef<FSlateStyleSet> Style = MakeShareable(new FSlateStyleSet("TerritoryEditorStyle"));

	FString path = FPaths::GameSourceDir() + "Editor/TerritoryEditor/Resource/";
	Style->SetContentRoot(path);
	
	// Icon 
	FString iconPath = Style->RootToContentDir(TEXT("TerritoryEditor_icon"), TEXT(".png"));	
	Style->Set("TerritoryEditor.OpenTerritoryEditor", new FSlateImageBrush(iconPath, Icon40x40));	
	Style->Set("TerritoryEditor.OpenTerritoryEditor.Small", new FSlateImageBrush(iconPath, Icon20x20));
...
const ISlateStyle& FGsTerritoryEditorStyle::Get()
{
